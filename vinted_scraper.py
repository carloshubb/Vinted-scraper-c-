...
# Combos with updated catalog_ids and brand_ids (verify these!)
combos = [
...
# combos = [
#     {"brand": "Zara", "category": "Vestidos", "audience": "Mujer", "catalog_ids": [10], "brand_ids": [12]},
#     {"brand": "Mango", "category": "Vestidos", "audience": "Mujer", "catalog_ids": [10], "brand_ids": [15]},
#     {"brand": "Nike", "category": "Zapatillas", "audience": "Hombre", "catalog_ids": [1242], "brand_ids": [53]},
#     {"brand": "H&M", "category": "Camiseta", "audience": "Hombre", "catalog_ids": [77], "brand_ids": [7]},
#     {"brand": "Levi's", "category": "Vaqueros", "audience": "Hombre", "catalog_ids": [257], "brand_ids": [10]}
# ]
...
def scrape_vinted(headless=True):
            ...
            while True:
                ...
                for attempt in range(retries):
                    try:
                        ...
                        # json_filename = f"vinted_scrape_{timestamp}.json"
                        # json_filepath = os.path.join(output_dir, json_filename)
                        # debug_filename = f"debug_response_{'brand'}_page{page}_{timestamp}.json"
                        # debug_filepath = os.path.join(output_dir, debug_filename)

                        # with open(debug_filepath, "w", encoding="utf-8") as f:
                        #     json.dump(response, f, ensure_ascii=False, indent=2)

                        # logger.info(f"Saved debug response to {debug_filepath}")
                        ...
                        for item in items:
                            ...
                            size_raw = item.get('size_title', item.get('size', {}).get('title', '')) if item.get('size') or item.get('size_title') else ''
                            condition_raw = item.get('status', '')
                            price_dict = item.get('price', {})
                            price = float(price_dict.get('amount', '0').replace(',', '.')) if isinstance(price_dict, dict) and price_dict.get('amount') else 0.0
                            ...
                            listing_url = item.get('url', f"https://www.vinted.es/items/{item.get('id', '')}")
                        ...
                        if total_pages:
                            ...
                            if page_num >= total_pages:
                                ...
                                break
                        elif not pagination.get('next_page') or len(items) < 96:
                        ...
                        if attempt == retries - 1:
                            ...
                            break
                if not items or (total_pages and page_num >= total_pages) or not pagination.get('next_page'):
                    break
                time.sleep(5)
    ...
    # ---------- Save CSV ----------
    csv_filename = f"vinted_scrape_{timestamp}.csv"
    csv_filepath = os.path.join(output_dir, csv_filename)

    df = pd.DataFrame(data)
    df.to_csv(csv_filepath, index=False, encoding='utf-8')
    logger.info(f"Saved {len(data)} items to {csv_filepath}")
    ...
    logger.info(f"Saved {len(data)} items to {json_filepath}")
    output_dir = os.path.join(os.getcwd(), "data")
